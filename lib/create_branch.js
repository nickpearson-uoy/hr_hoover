...
export default async function create_branch( page , parent_id , path = '' )
{
	...
	.then( response =>
	{
		...
		{
			throw( `⚠️ Error creating ${ section_path }: got response code ${ response.status }` );
		}
	...
	.then( section_id =>
	{
		console.log( `- ${ section_path } - ${ section_id }` );

		// Apply the output URI

		if( page.slug )
		{
			return set_section_options( section_id , page )
		}
...
function set_section_options( section_id , page )
{
	return new Promise( ( resolve , reject ) =>
	{
		...
		.then( section =>
		{
			section[ 'output-uri' ] = page.slug;
			section[ 'show' ] = page.show_in_nav;
			section[ 'metaDatas' ] = [ { id: 6 , value: ( page?.meta?.description ? page.meta.description : '' ) , lang: 'en' } ];
	...
	return new Promise( ( resolve , reject ) =>
	{
		...
		const elements = Object.keys( content_type.fields ).reduce( ( _elements , field_key ) =>
		{
			if( typeof content.data[ field_key ] !== 'undefined' )
			{
				...
				_elements[ content_type.fields[ field_key ] ] = _elements[ content_type.fields[ field_key ] ].replace( /\<a class\=\"button\"/g , '<a class="uoy_cta"' );
